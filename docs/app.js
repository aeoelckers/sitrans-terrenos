...
function buildGeographyLookups() {
  ...
  Object.entries(COMMUNES_BY_REGION).forEach(([region, communes]) => {
    const set = new Set(communes);
    communesByRegion.set(region, set);
    communes.forEach((commune) => allCommunes.add(commune));
  });
  ...
  listings.forEach((item) => {
    ...
    if (region) {
      if (!communesByRegion.has(region)) {
        communesByRegion.set(region, new Set());
      }
      if (commune) {
        communesByRegion.get(region).add(commune);
...
function populateOptions(select, values, previousValue = '') {
  ...
  const collection = Array.from(values);
  ...
  if (previousValue && collection.includes(previousValue)) {
